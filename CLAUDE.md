...
его писем и дневников (тома 18–22 собрания сочинений в EPUB). Поиск релевантных
фрагментов — локально; генерация ответа — через внешнюю LLM с системным промптом
«говори как Толстой».

## Архитектура

```
Mac (разработка, разовая подготовка)      VPS (прод, Docker)
...
Ключевые решения:
- **Эмбеддинги — локально**, одна модель на Mac и на сервере. Векторы документов
  считаются один раз на Mac; на сервере эмбеддится только запрос.
- **Поиск — numpy** (brute-force косинус, ~5.7 тыс. фрагментов = доли мс). БД нет.
- **Генерация — любой OpenAI-совместимый API** (эндпоинт в `.env`).
- **Mac = HTTP без Docker; VPS = Docker + HTTPS** (самоподписанный серт на IP, 8443).
...
| Путь | Назначение |
|---|---|
| `scripts/epub_to_txt.py` | EPUB (тома 18–22) → `data/*.txt`, отделяет текст от сносок |
| `scripts/corpus.py` | чтение txt, нарезка на чанки + метаданные |
| `scripts/embed.py` | общий модуль эмбеддингов (Mac и сервер) |
...
- `index/chunks.json` — тексты фрагментов + метаданные (адресат, год, тип).
- Связь по позиции: i-й вектор ↔ i-й элемент chunks.json.
- Собирается на Mac (`make embeddings`), переносится на сервер (rsync/scp).
...
- **Совместимость эмбеддингов**: модель И версия `fastembed` на сервере должны
  совпадать с теми, чем собран индекс на Mac (иначе pooling разойдётся, поиск
